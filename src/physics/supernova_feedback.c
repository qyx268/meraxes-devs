                                        ...
                                        double new_metals)
{
  ...
  gal->StellarMass = check_float_cast((double)(gal->StellarMass) - (m_recycled), FloatField_StellarMass);
#if USE_MINI_HALOS
  gal->StellarMass = check_float_cast((double)(gal->StellarMass) - (m_remnant), FloatField_StellarMass);
  gal->StellarMass_II = check_float_cast((double)(gal->StellarMass_II) - (m_recycled_II), FloatField_StellarMass_II);
  gal->StellarMass_III = check_float_cast((double)(gal->StellarMass_III) - ((m_recycled_III + m_remnant)), FloatField_StellarMass_III);
  gal->Remnant_Mass = check_float_cast((double)(gal->Remnant_Mass) + (m_remnant), FloatField_Remnant_Mass);
#endif
  ...
  // properties.
  gal->MetalsStellarMass = check_float_cast((double)(gal->MetalsStellarMass) - (new_metals), FloatField_MetalsStellarMass);
  gal->ColdGas = check_float_cast((double)(gal->ColdGas) + (m_recycled), FloatField_ColdGas);
  ...
  if (gal->ColdGas > 1e-10)
    gal->MetalsColdGas = check_float_cast((double)(gal->MetalsColdGas) + (new_metals), FloatField_MetalsColdGas);
  else
    central->MetalsHotGas = check_float_cast((double)(central->MetalsHotGas) + (new_metals), FloatField_MetalsHotGas);
  ...
  gal->MetalsColdGas = check_float_cast((double)(gal->MetalsColdGas) - (m_reheat * metallicity * transferred_fraction), FloatField_MetalsColdGas);
  central->MetalsHotGas = check_float_cast((double)(central->MetalsHotGas) + (m_reheat * metallicity * transferred_fraction), FloatField_MetalsHotGas);
  ...
  if (!gal->ghost_flag) {
    ...
    central->HotGas = check_float_cast((double)(central->HotGas) - (m_eject), FloatField_HotGas);
    central->MetalsHotGas = check_float_cast((double)(central->MetalsHotGas) - (m_eject * metallicity), FloatField_MetalsHotGas);
    central->EjectedGas = check_float_cast((double)(central->EjectedGas) + (m_eject), FloatField_EjectedGas);
    central->MetalsEjectedGas = check_float_cast((double)(central->MetalsEjectedGas) + (m_eject * metallicity), FloatField_MetalsEjectedGas);
  }
                                                    ...
                                                    // bubble will overtake a previous one in no more than 17 snapshots!
{
  ...
  if (gal->RmetalBubble > 0.) {
    if ((gal->RmetalBubble <= gal->Rvir) && (gas_density >= IGM_density))
      gal->RmetalBubble = check_float_cast((double)(gal->PrefactorBubble * pow(gas_density, -0.2) *
                          pow((gal->TimeBubble - run_globals.LTTime[snapshot] * time_unit), 0.4)), FloatField_RmetalBubble);
    else
      gal->RmetalBubble = check_float_cast((double)(gal->PrefactorBubble * pow(IGM_density, -0.2) *
                          pow((gal->TimeBubble - run_globals.LTTime[snapshot] * time_unit), 0.4)), FloatField_RmetalBubble);
  }
  ...
  if (gal->Type == 0) {
    for (int i_burst = 0; i_burst < n_bursts; i_burst++) {
      ...
      if (i_burst != 0) {
        gal->Prefactor[n_bursts - i_burst] = check_float_cast((double)(gal->Prefactor[n_bursts - i_burst - 1]), FloatField_Prefactor);
        gal->Times[n_bursts - i_burst] = check_float_cast((double)(gal->Times[n_bursts - i_burst - 1]), FloatField_Times);
        if (gal->Prefactor[n_bursts - i_burst] > 0.0) {
          if ((gal->Radii[n_bursts - i_burst] >= gal->Rvir) || (IGM_density >= gas_density))
            gal->Radii[n_bursts - i_burst] = check_float_cast((double)(gal->Prefactor[n_bursts - i_burst] * pow(IGM_density, -0.2) *
              pow((gal->Times[n_bursts - i_burst] - run_globals.LTTime[snapshot] * time_unit), 0.4)), FloatField_Radii);
          else
            gal->Radii[n_bursts - i_burst] = check_float_cast((double)(gal->Prefactor[n_bursts - i_burst] * pow(gas_density, -0.2) *
              pow((gal->Times[n_bursts - i_burst] - run_globals.LTTime[snapshot] * time_unit), 0.4)), FloatField_Radii);
        } else
          gal->Radii[n_bursts - i_burst] = check_float_cast((double)(0.0), FloatField_Radii);
        if (gal->Radii[n_bursts - i_burst] > gal->RmetalBubble) {
          ...
          gal->RmetalBubble = check_float_cast((double)(gal->Radii[n_bursts - i_burst]), FloatField_RmetalBubble);
          gal->PrefactorBubble = check_float_cast((double)(gal->Prefactor[n_bursts - i_burst]), FloatField_PrefactorBubble);
          gal->TimeBubble = check_float_cast((double)(gal->Times[n_bursts - i_burst]), FloatField_TimeBubble);
        }
      }
    }
  }
  if (!gal->ghost_flag) {
    gal->Prefactor[0] = check_float_cast((double)(pow(sn_energy / PROTONMASS, 0.2) / UnitLength_in_cm), FloatField_Prefactor); // Mpc s^-0.4
    gal->Times[0] = check_float_cast((double)(run_globals.LTTime[snapshot] * time_unit), FloatField_Times);                // s
  } else {
    gal->Prefactor[0] = check_float_cast((double)(0.0), FloatField_Prefactor);
    gal->Times[0] = check_float_cast((double)(0.0), FloatField_Times);
  }
  gal->Radii[0] = check_float_cast((double)(0.0), FloatField_Radii);
}
#endif
